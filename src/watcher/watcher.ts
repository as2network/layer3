...
type WatcherAnchorState = MappedState<WatcherAppointmentAnchorState> &
  BlockNumberState;

export class WatcherAppointmentStateReducer
  implements StateReducer<WatcherAppointmentAnchorState, IBlockStub & Logs> {
  constructor(
    private cache: ReadOnlyBlockCache<IBlockStub & Logs>,
    private appointment: IEthereumAppointment
  ) {
    const filter = this.appointment.getEventFilter();
    if (!filter.topics)
      throw new ApplicationError(`topics should not be undefined`);
  }
  public getInitialState(
    block: IBlockStub & Logs
  ...
  public reduce(
    prevState: WatcherAppointmentAnchorState,
    block: IBlockStub & Logs
  ): WatcherAppointmentAnchorState {
    if (
...
export class Watcher extends Component<WatcherAnchorState, IBlockStub & Logs> {
    ...
    super(
      new MappedStateReducer(
        ...
        (appointment: IEthereumAppointment) =>
          new WatcherAppointmentStateReducer(blockCache, appointment),
  ...
  private shouldHaveStartedResponder = (
    ...
    state.blockNumber - appointmentState.blockObserved + 1 >=
      this.confirmationsBeforeResponse;
  ...
  private shouldRemoveAppointment = (
    ...
    appointmentState.state === WatcherAppointmentState.OBSERVED &&
    state.blockNumber - appointmentState.blockObserved + 1 >=
      this.confirmationsBeforeRemoval;
  ...
  public async handleChanges(
    ...
    for (const [appointmentId, appointmentState] of state.items.entries()) {
      ...
      if (
        (!prevWatcherAppointmentState ||
          prevWatcherAppointmentState.state ===
            WatcherAppointmentState.WATCHING) &&
        appointmentState.state === WatcherAppointmentState.OBSERVED
      ) {
        logger.info(
          `Observed appointment ${appointmentId} in block ${appointmentState.blockObserved}.`
        );
      }

      // Start response if necessary
      if (
        !this.shouldHaveStartedResponder(
          prevState,
          prevWatcherAppointmentState
        ) &&
        ...
        logger.info(
          `Responding to appointment ${appointmentId}, block ${state.blockNumber}.`
        );
      ...
      if (
        ...
        logger.info(
          `Removing appointment ${appointmentId}, block ${state.blockNumber} from watcher.`
        );
