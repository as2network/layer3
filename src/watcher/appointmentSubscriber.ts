...
export class AppointmentSubscriber {
  ...
  public subscribe(
    ...
    if (this.provider.listenerCount(filter) !== 0) {
      ...
      const currentListener = this.provider.listeners(
        filter
      )[0] as IAppointmentListener;
      throw new ApplicationError(
        `Only one appointment should be subscribed to a given filter at any one time. Appointment: ${appointmentId} cannot be subscribed since appointment: ${currentListener.appointmentId} is already subscribed.`
      );
    }
    ...
    const listenerAndAppointment = Object.assign(listener.bind({}), {
      appointmentId,
    });
