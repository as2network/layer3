...
export class AppointmentStore extends StartStopService
  implements IAppointmentStore {
  constructor(
    private readonly db: LevelUp<encodingDown<string, any>>,
    private readonly appointmentConstructors: Map<
      ChannelType,
      (obj: any) => IEthereumAppointment
    >
  ) {
  ...
  protected async startInternal() {
    ...
    for await (const record of this.db.createValueStream()) {
      ...
      if (!constrctr)
        throw new ConfigurationError(`Unrecognised channel type: ${type}.`);

      const appointment = constrctr(record);
      // // add too the indexes
      this.mAppointmentsById.set(appointment.id, appointment);
      this.appointmentsByStateLocator[
  ...
  private readonly mAppointmentsById: Map<
    string,
    IEthereumAppointment
  > = new Map();
   ...
   * @param appointment
   */
  public addOrUpdateByStateLocator(
    appointment: IEthereumAppointment
  ): Promise<boolean> {
    // As we are accessing data structures by state locator, we make sure to acquire a lock on it
    return this.stateLocatorLockManager.withLock(
      appointment.getStateLocator(),
      async () => {
        const currentAppointment = this.appointmentsByStateLocator[
          appointment.getStateLocator()
        ];
        // is there a current appointment
        if (currentAppointment) {
          if (
            currentAppointment.getStateNonce() >= appointment.getStateNonce()
          ) {
            this.logger.info(
        ...
        const batch = this.db
          .batch()
          .put(appointment.id, appointment.getDBRepresentation());
