...
export class ConfigManager {
  public static PisaConfigProperties = [
    ...
    new ConfigProperty("loglevel", (config) => config.loglevel, {
      description:
        "Verbosity of the logs. Accepted values by increasing verbosity: " +
        Object.values(LogLevel).join(", "),
      string: true,
    }),
    new ConfigProperty(
      "rate-limit-user-window-ms",
      (config) => config.rateLimitUserWindowMs,
      {
    ...
    new ConfigProperty(
      "rate-limit-user-max",
      (config) => config.rateLimitUserMax,
      {
    ...
    new ConfigProperty(
      ...
      (config) => config.rateLimitUserMessage,
      {
    ...
    new ConfigProperty(
      "rate-limit-global-window-ms",
      (config) => config.rateLimitGlobalWindowMs,
      {
    ...
    new ConfigProperty(
      ...
      (config) => config.rateLimitGlobalMax,
      {
    ...
    new ConfigProperty(
      "rate-limit-global-message",
      (config) => config.rateLimitGlobalMessage,
      {
    ...
    new ConfigProperty(
      ...
      {
        description:
          "The maximum depth of reorg that the application can handle. Eg. 100. Max is 200.",
        number: true,
      }
    ),
    new ConfigProperty(
      "watcher-response-confirmations",
      (config) => config.rateLimitGlobalMessage,
      {
        description:
          "The number of confirmations on an event before the watcher responds.",
        number: true,
      }
    ),
  ];
  ...
  public fromCommandLineArgs(argv: string[]) {
    ...
    let commandLineConfig = yargs.scriptName("pisa").usage("$0 [args]").help();

    // add each of the props
    this.properties.forEach(
      (p) =>
        (commandLineConfig = commandLineConfig.option(
          p.commandLineName,
          p.yargConfig
        ))
    );
