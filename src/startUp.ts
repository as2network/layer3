...
function checkArgs(args: IArgConfig) {
  if (
    ...
    console.error(
      "Options 'rate-limit-user-windowms' and 'rate-limit-user-max' must be provided together."
    );
    process.exit(1);
  }

  if (
    (commandLineConfig.rateLimitGlobalWindowMs &&
      !commandLineConfig.rateLimitGlobalMax) ||
    (!commandLineConfig.rateLimitGlobalWindowMs &&
      commandLineConfig.rateLimitGlobalMax)
  ) {
    console.error(
      "Options 'rate-limit-global-windowms' and 'rate-limit-global-max' must be provided together."
    );
    process.exit(1);
  }
}
...
function checkLogLevel(logLevel: string): LogLevelInfo {
  ...
  if (!logLevelInfo) {
    console.error(
      "Option 'loglevel' can only be one of the following: " +
...
async function startUp() {
  ...
  const db = levelup(
    encodingDown(leveldown(config.dbDir), { valueEncoding: "json" })
  );
