...
export class GasQueue {
  ...
  public constructor(
    ...
    if (replacementRate < 1)
      throw new ArgumentError(
        "Replacement rate should be positive.",
        replacementRate
      );
    if (emptyNonce < 0)
      throw new ArgumentError("Nonce must not be negative.", emptyNonce);
    if (maxQueueDepth < 1)
      throw new ArgumentError(
        "Max queue depth must be greater than 0.",
        maxQueueDepth
      );
    ...
    if (queueItems.length > 1) {
      ...
      for (let index = 1; index < queueItems.length; index++) {
        ...
        if (item.nonce !== prevItem.nonce + 1) {
          throw new ArgumentError(
            "Nonce of queue item did not increase by one.",
            queueItems
          );
        }
        ...
        if (
          queueItems.find(
            (q, i) =>
              q.request.identifier.equals(item.request.identifier) &&
              i !== index
          )
        ) {
          throw new ArgumentError(
    ...
    if (
      queueItems.length > 0 &&
      queueItems[queueItems.length - 1].nonce + 1 !== emptyNonce
    ) {
      throw new ArgumentError(
    ...
    if (this.queueItems.length > this.maxQueueDepth) {
      throw new ArgumentError(
        `Cannot create queue. Max queue depth of ${this.maxQueueDepth} reached.`
      );
    }
  }
  ...
  private getReplacementGasPrice(
  ...
  private append(queueItems: GasQueueItem[], request: GasQueueItemRequest) {
    const queueItem = new GasQueueItem(
      request,
   ...
   */
  private shiftRight(
    queueItems: GasQueueItem[],
    startIndex: number,
    endIndex: number
  ) {
    if (startIndex > endIndex) {
      throw new ArgumentError(
        "Start index must be less than or equal end index.",
        startIndex,
        endIndex
      );
    }
    ...
    for (let queueIndex = endIndex; queueIndex >= startIndex; queueIndex--) {
      ...
      if (nextIndex !== queueItems.length)
        this.replace(queueItems, nextIndex, previousItemRequest);
  ...
  private replace(
    ...
    const replacementPrice = this.getReplacementGasPrice(
      replacementItem.nonceGasPrice,
    ...
    const newGasPrice = replacementPrice.gt(request.idealGasPrice)
      ? replacementPrice
      : request.idealGasPrice;

    queueItems[index] = new GasQueueItem(
      request,
      newGasPrice,
      request.idealGasPrice,
      nonce
  ...
  public add(request: GasQueueItemRequest): GasQueue {
    if (this.depthReached()) {
      throw new ArgumentError(
        `Cannot add item. Max queue depth reached.`,
        this.maxQueueDepth
      );
    }
    ...
    const foundTxIndex = this.queueItems.findIndex((indexedTx) =>
      request.idealGasPrice.gt(indexedTx.idealGasPrice)
    );
  ...
  public consume(identifier: PisaTransactionIdentifier) {
    const index = this.queueItems.findIndex((item) =>
      identifier.equals(item.request.identifier)
    );
    if (index === -1)
      throw new ArgumentError("Identifier not found in queue.", identifier);
    const clonedArray = this.cloneQueueItems();
  ...
  public contains(identifier: PisaTransactionIdentifier): boolean {
    return (
      this.queueItems.findIndex((item) =>
        item.request.identifier.equals(identifier)
      ) !== -1
    );
  }
  ...
  public prepend(lowerNonceItems: GasQueueItem[]): GasQueue {
    ...
    const allItemsOrderedByNonce = lowerNonceItems
      .concat(this.queueItems)
