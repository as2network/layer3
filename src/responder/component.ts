...
export type ResponderAppointmentAnchorState =
  | PendingResponseState
  | MinedResponseState;
export type ResponderAnchorState = MappedState<
 ...
 */
export class ResponderAppointmentReducer
  implements StateReducer<ResponderAppointmentAnchorState, ResponderBlock> {
  public constructor(
  ...
  private txIdentifierInBlock(
    ...
    for (const tx of block.transactions) {
      ...
      if (
        txIdentifier.equals(identifier) &&
  ...
  public reduce(
    prevState: ResponderAppointmentAnchorState,
    block: ResponderBlock
  ): ResponderAppointmentAnchorState {
 ...
 */
export class MultiResponderComponent extends Component<
...
> {
  ...
  private shouldAppointmentBeRemoved = (
    ...
    appointmentState != undefined &&
    appointmentState.kind === ResponderStateKind.Mined &&
    state.blockNumber - appointmentState.blockMined >
    ...
    for (const [appointmentId, currentItem] of state.items.entries()) {
      ...
      if (!prevItem) {
        ...
        if (currentItem.kind === ResponderStateKind.Mined) {
          logger.info(
            `Initial mined transaction ${JSON.stringify(currentItem)}.`
          );
        ...
        if (
          prevItem.kind === ResponderStateKind.Pending &&
          currentItem.kind === ResponderStateKind.Mined
        ) {
      ...
      if (
        !this.hasResponseBeenMined(prevItem) &&
        this.hasResponseBeenMined(currentItem)
      ) {
