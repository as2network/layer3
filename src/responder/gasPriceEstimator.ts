...
export class ExponentialCurve {
  ...
  constructor(
    ...
    if (y1 === 0 || y2 == 0)
      throw new ArgumentError("Y values cannot equal zero.", y1, y2);
    if ((y1 < 0 && y2 > 0) || (y1 > 0 && y2 < 0)) {
      throw new ArgumentError(
        "Y values most all be positive or all be negative.",
        y1,
        y2
      );
    }
    if (x1 === x2)
      throw new ArgumentError(
        "x1 cannot equal x2 for exponential curve.",
        x1,
        x2
      );
...
export class ExponentialGasCurve {
  ...
  constructor(public readonly currentGasPrice: BigNumber) {
    if (currentGasPrice.lt(0))
      throw new ArgumentError("Gas price cannot be less than zero.");
    ...
    const x2 =
      ExponentialGasCurve.MEDIAN_BLOCKS + ExponentialGasCurve.AVERAGE_TO_MINE;
  ...
  public getGasPrice(blocksUntilDeadline: number) {
    if (blocksUntilDeadline < 0)
      throw new ArgumentError(
        "blocksUntilDeadline cannot be less than zero.",
        blocksUntilDeadline
      );
