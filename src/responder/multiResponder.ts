...
export class MultiResponder extends StartStopService {
  ...
  public readonly respondedTransactions: Map<
    string,
    { id: string; queueItem: GasQueueItem }
  > = new Map();
  ...
  public constructor(
    ...
    if (replacementRate < 0)
      throw new ArgumentError(
        "Cannot have negative replacement rate.",
        replacementRate
      );
    if (maxConcurrentResponses < 1) {
      throw new ArgumentError(
        "Maximum concurrent requests must be greater than 0.",
  ...
  public async startResponse(
    ...
    try {
      ...
      const data = abiInterface.functions[responseData.functionName].encode(
        responseData.functionArgs
      );
      ...
      logger.info(
        `Enqueueing request for ${appointmentId}. ${JSON.stringify(request)}.`
      );
      ...
      if (doh instanceof QueueConsistencyError) logger.error(doh.stack!);
        ...
        logger.error(
          `Unexpected error trying to respond for: ${appointmentId}.`
        );
  ...
  public async txMined(txIdentifier: PisaTransactionIdentifier, nonce: number) {
    try {
      ...
      if (this.mQueue.queueItems.length === 0) {
        throw new QueueConsistencyError(
          `Transaction mined for empty queue at nonce ${nonce}. ${inspect(
            txIdentifier
          )}`
        );
      }
      if (
        this.mQueue.queueItems.findIndex((item) =>
          item.request.identifier.equals(txIdentifier)
        ) === -1
      ) {
        throw new QueueConsistencyError(
      ...
      if (frontItem.nonce !== nonce) {
        throw new QueueConsistencyError(
          `Front of queue nonce ${
            frontItem.nonce
          } does not correspond to nonce ${nonce}. ${inspect(txIdentifier)}`
        );
      }
      ...
      if (doh instanceof QueueConsistencyError) logger.error(doh.stack!);
        ...
        logger.error(
          `Unexpected error after mining transaction. ${txIdentifier}.`
        );
        if (doh.stack) logger.error(doh.stack);
        else logger.error(doh);
      }
  ...
  public async reEnqueueMissingItems(appointmentIdsStillPending: string[]) {
    ...
    const missingQueueItems = appointmentIdsStillPending
      ...
      .map((txRecord) => {
        if (!txRecord)
          throw new ArgumentError(
            "No record of appointment in responder.",
            txRecord
          );
