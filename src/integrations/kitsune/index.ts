  ...
  constructor(obj: any) {
    if (KitsuneAppointment.checkKitsuneAppointment(obj)) {
      ...
      throw new ConfigurationError(
        "User defined type guard failed to throw for KitsuneAppointment"
      );
  }
...
export class KitsuneInspector extends Inspector<KitsuneAppointment> {
  ...
  public async verifyAppointment(
    ...
    logger.info(
      appointment.formatLog(`On-chain round: ${currentChannelRound}`)
    );
    ...
    const channelDisputePeriod: number = await contract.disputePeriod();
    logger.info(
      appointment.formatLog(`On-chain dispute period: ${channelDisputePeriod}.`)
    );
    ...
    logger.info(
      appointment.formatLog(`On-chain channel status: ${channelStatus}.`)
    );
    // ON = 0, DISPUTE = 1, OFF = 2
    if (channelStatus != 0) {
      throw new PublicInspectionError(
        `Channel status is ${channelStatus} not 0.`
      );
    }

    //verify all the signatures
    // get the participants
    let participants: string[] = (await [
      await contract.plist(0),
      await contract.plist(1),
    ]) as string[];
    logger.info(
      appointment.formatLog(
        `On-chain participants: ${JSON.stringify(participants)}.`
      )
    );

    // form the hash
    const setStateHash = KitsuneTools.hashForSetState(
      hState,
      appointmentRound,
      contract.address
    );
  ...
  private verifySignatures(
    ...
    const signers = sigs.map((sig) =>
      verifyMessage(ethers.utils.arrayify(message), sig)
    );
    participants.forEach((party) => {
      const signerIndex = signers
        .map((m) => m.toLowerCase())
        .indexOf(party.toLowerCase());
      if (signerIndex === -1) {
        throw new PublicInspectionError(
          `Party ${party} not present in signatures.`
        );
      }
