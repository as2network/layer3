...
export class RaidenAppointment extends EthereumAppointment {
  ...
  private static checkRaidenStateUpdate(obj: any) {
    if (!obj)
      throw new PublicDataValidationError("stateUpdate does not exist.");
    ...
    if (hexLength !== 32) {
      throw new PublicDataValidationError(
        `Invalid bytes32: ${obj.additional_hash}`
      );
    }
    ...
    if (balanceHexLength !== 32) {
      throw new PublicDataValidationError(
        `Invalid bytes32: ${obj.balanceHexLength}`
      );
    }
    ...
    try {
      ...
      throw new PublicDataValidationError(
        `${obj.closing_participant} is not a valid address.`
      );
    }
    ...
    try {
      ...
      throw new PublicDataValidationError(
        `${obj.non_closing_participant} is not a valid address.`
      );
    }
    ...
    try {
      ...
      throw new PublicDataValidationError(
        `${obj.token_network_identifier} is not a valid address.`
      );
    }
  }
  ...
  public getStateLocator() {
    ...
    return `${this.stateUpdate.token_network_identifier}:${this.stateUpdate.channel_identifier}:${this.stateUpdate.closing_participant}`;
  }

  public getEventFilter(): ethers.EventFilter {
    let event = new ethers.utils.Interface(this.getContractAbi());
    let topics = event.events["ChannelClosed"].encodeTopics([
...
export class RaidenInspector extends Inspector<RaidenAppointment> {
  ...
  public async checkInspection(appointment: RaidenAppointment) {
    ...
    if (code != RaidenTools.ContractDeployedBytecode) {
      throw new PublicInspectionError(
        `Contract at: ${contractAddress} does not have correct bytecode.`
      );
    }
    ...
    logger.info(
      appointment.formatLog(`On-chain round: ${nonce.toString(10)}.`)
    );
    ...
    if (channelStatus != 1) {
      throw new PublicInspectionError(
        `Channel status is ${channelStatus} not "Opened".`
      );
    }
    ...
    logger.info(
      appointment.formatLog(
        `On-chain dispute period: ${channelDisputePeriod.toString(10)}.`
      )
    );
    if (channelDisputePeriod <= this.minimumDisputePeriod) {
      throw new PublicInspectionError(
        `Channel dispute period ${channelDisputePeriod} is less than or equal the minimum acceptable dispute period ${this.minimumDisputePeriod}.`
      );
    }
    ...
    if (appointment.expiryPeriod <= channelDisputePeriod - currentBlockNumber) {
      throw new PublicInspectionError(
        `Supplied appointment expiryPeriod ${appointment.expiryPeriod} is not greater than the channel dispute period ${channelDisputePeriod}.`
      );
    }
    ...
    if (appointment.stateUpdate.non_closing_participant !== nonClosingAccount) {
      throw new PublicInspectionError(
        `Supplied non_closing_signature was created by account ${nonClosingAccount}, not account ${appointment.stateUpdate.non_closing_participant}.`
      );
    }
    ...
    if (appointment.stateUpdate.closing_participant !== closingAccount) {
      throw new PublicInspectionError(
        `Supplied closing_signature was created by account ${closingAccount}, not account ${appointment.stateUpdate.closing_participant}.`
      );
    }
