...
export class BlockchainMachine<
  TBlock extends IBlockStub
> extends StartStopService {
  ...
  public addComponent(component: Component<{}, TBlock>): void {
    if (this.started) {
      throw new ApplicationError(
        "Components must be added before the BlockchainMachine is started."
      );
    }
  ...
  private processNewBlock(block: TBlock) {
    ...
    for (const { component, states } of this.componentsAndStates) {
      ...
      if (this.blockProcessor.blockCache.hasBlock(block.parentHash)) {
        const parentBlock = this.blockProcessor.blockCache.getBlockStub(
          block.parentHash
        );
