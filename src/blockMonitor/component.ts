...
> implements StateReducer<MappedState<TMappedState>, TBlock> {
  ...
  constructor(
    ...
    public readonly getBaseReducer: (
      obj: TMappedType
    ) => StateReducer<TMappedState, TBlock>,
   ...
   */
  public reduce(
    prevState: MappedState<TMappedState> & TState,
    block: TBlock
  ): MappedState<TMappedState> & TState {
    const items: Map<string, TMappedState> = new Map();
    for (const obj of this.getItems()) {
      const baseReducer = this.getBaseReducer(obj);
 ...
 */
export class BlockNumberReducer
  implements StateReducer<BlockNumberState, IBlockStub> {
