 ...
 **/
export class BlockCache<TBlock extends IBlockStub>
  implements ReadOnlyBlockCache<TBlock> {
  ...
  private removeBlock(blockHash: string) {
    ...
    if (!block) {
      ...
      throw new ApplicationError(
        `Block with hash ${blockHash} not found, but it was expected.`
      );
    }
  ...
  public canAddBlock(block: Readonly<TBlock>): boolean {
    return (
      this.isEmpty ||
      this.hasBlock(block.parentHash) ||
      block.number <= this.minHeight
    );
  }
  ...
  public addBlock(block: Readonly<TBlock>): boolean {
    ...
    if (!this.canAddBlock(block)) {
      throw new ApplicationError(
        "Tried to add a block before its parent block."
      );
    }
  ...
  public getBlockStub(blockHash: string): Readonly<TBlock> {
    ...
    if (!blockStub)
      throw new ApplicationError(`Block not found for hash: ${blockHash}.`);
    return blockStub;
  }
  ...
  public *ancestry(
    initialBlockHash: string
  ): IterableIterator<Readonly<TBlock>> {
  ...
  public getOldestAncestorInCache(blockHash: string): Readonly<TBlock> {
    if (!this.hasBlock(blockHash)) {
      throw new ArgumentError(
        `The block with hash ${blockHash} is not in cache`
      );
    }

    // Find the deepest ancestor that is in cache
    const result = this.findAncestor(
      blockHash,
      (block) => !this.hasBlock(block.parentHash)
    );

    if (!result) {
      // This can never happen, since blockHash already satisfies the predicate in findAncestor
      throw new ApplicationError(
        "An error occurred while searching for the oldest ancestor in cache."
      );
    }
  ...
  public setHead(blockHash: string) {
    if (!this.hasBlock(blockHash)) {
      throw new ArgumentError(
        "Cannot set the head for a block that isn't in the cash.",
        blockHash
      );
    }
  ...
  public get head(): TBlock {
    if (this.headHash == null) {
      throw new ApplicationError(
        "Head used before the BlockCache is initialized."
      );
    }
...
export function getConfirmations<T extends IBlockStub & TransactionHashes>(
  ...
  const blockTxIsMinedIn = cache.findAncestor(headHash, (block) =>
    block.transactionHashes.includes(txHash)
  );
