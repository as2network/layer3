...
) => (blockNumberOrHash: number | string) => Promise<T>;

export const blockStubAndTxHashFactory = (
  provider: ethers.providers.Provider
) => async (
...
export class BlockProcessor<T extends IBlockStub> extends StartStopService {
  ...
  private processNewHead(headBlock: Readonly<T>) {
    ...
    const nearestEmittedBlockInAncestry = this.blockCache.findAncestor(
      headBlock.hash,
      (block) => this.emittedBlocks.has(block)
    );

    this.emit(
      BlockProcessor.NEW_HEAD_EVENT,
      headBlock,
      nearestEmittedBlockInAncestry
    );
