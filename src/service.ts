...
export class PisaService extends StartStopService {
  ...
  constructor(
    ...
    this.blockProcessor = new BlockProcessor<Block>(
      provider,
      blockFactory,
      blockCache
    );
    ...
    this.appointmentStore = new AppointmentStore(
      db,
      new Map(
        configs.map<[ChannelType, (obj: any) => IEthereumAppointment]>((c) => [
          c.channelType,
          c.appointment,
        ])
      )
    ...
    const watcher = new Watcher(
      ...
      config.watcherResponseConfirmations === undefined
        ? 5
        : config.watcherResponseConfirmations,
  ...
  private applyMiddlewares(app: express.Express, config: IArgConfig) {
    ...
    if (config.rateLimitGlobalMax && config.rateLimitGlobalWindowMs) {
      app.use(
        new rateLimit({
          ...
          message:
            config.rateLimitGlobalMessage ||
            "Server request limit reached. Please try again later.",
          windowMs: config.rateLimitGlobalWindowMs,
          max: config.rateLimitGlobalMax,
        })
      );
      this.logger.info(
        `Api global rate limit: ${config.rateLimitGlobalMax} requests every: ${
          config.rateLimitGlobalWindowMs / 1000
        } seconds.`
      );
    } else {
    ...
    if (config.rateLimitUserMax && config.rateLimitUserWindowMs) {
      ...
      this.logger.info(
        `Api per-user rate limit: ${config.rateLimitUserMax} requests every: ${
          config.rateLimitUserWindowMs / 1000
        } seconds.`
      );
    } else {
  ...
  private appointment(tower: PisaTower) {
    return async (
      ...
      if (!this.started) {
        this.logger.error(
          "Service initialising. Could not serve request: \n" +
            inspect(req.body)
        );
        res.status(503);
      ...
      try {
        ...
        if (doh instanceof PublicInspectionError)
          this.logAndSend(400, doh.message, doh, res);
        else if (doh instanceof PublicDataValidationError)
          this.logAndSend(400, doh.message, doh, res);
        else if (doh instanceof ApplicationError)
          this.logAndSend(500, doh.message, doh, res);
