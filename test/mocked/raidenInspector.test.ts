...
describe("Raiden inspector", () => {
  ...
  const appointmentObj = {
    ...
    stateUpdate: {
      additional_hash:
        "0xbea4d9b77acbfb279382b573824ec01ebc6b2185ccd0db2d967ba600250bf9f4",
      balance_hash:
        "0xe73f895f3e0439ede072b30229eac682208f45c1e1d939f2e8d3a71980778765",
  ...
  before(() => {
    ...
    mockito
      .when(mockedProvider.getCode(tokenNetworkAddress))
      .thenResolve(RaidenTools.ContractDeployedBytecode);
    mockito
      .when(mockedProvider.getCode(otherTokenNetwork))
      .thenResolve(RaidenTools.ContractDeployedBytecode);
    mockito.when(mockedProvider.getCode(party1Address)).thenResolve("0x");
    mockito
      .when(mockedProvider.getCode(party2Address))
      .thenResolve(
        RaidenTools.ContractDeployedBytecode.slice(
          0,
          RaidenTools.ContractDeployedBytecode.length - 20
        )
      );
    mockito
      .when(mockedProvider.getBlockNumber())
      .thenResolve(onChainBlockNumber);
    provider = mockito.instance(mockedProvider);

    const contractMock = mockito.mock(ethers.Contract);
    const getChannelParticipantInfo = () =>
      new Promise((resolve) => resolve(["", "", "", "", onChainNonce]));
    const getChannelInfo = () =>
      new Promise((resolve) =>
        resolve([onChainSettleBlockNumber, onChainStatus])
      );
    ...
    mockito
      .when(
        new spiedEthers.Contract(
          tokenNetworkAddress,
          RaidenTools.ContractAbi,
          provider
        )
      )
      .thenReturn(contract);
  ...
  it("throws for wrong additional hash", async () => {
    ...
    const wrongAdditionalHash =
      "0xaea4d9b77acbfb279382b573824ec01ebc6b2185ccd0db2d967ba600250bf9f4";
    const appointment = new RaidenAppointment({
      ...appointmentObj,
      stateUpdate: {
        ...appointmentObj.stateUpdate,
        additional_hash: wrongAdditionalHash,
      },
    });
  ...
  it("throws for wrong balance hash", async () => {
    ...
    const wrongBalanceHash =
      "0xa73f895f3e0439ede072b30229eac682208f45c1e1d939f2e8d3a71980778765";
    const appointment = new RaidenAppointment({
      ...appointmentObj,
      stateUpdate: {
        ...appointmentObj.stateUpdate,
        balance_hash: wrongBalanceHash,
      },
    });
    const inspector = new RaidenInspector(minimumDisputePeriod, provider);
    expect(inspector.checkInspection(appointment)).to.be.rejected;
  ...
  it("throws for incorrect closing participant", async () => {
    ...
    const wrongClosingParticipant =
      "0xccca21b97b27DefC210f01A7e64119A784424D26";
    const appointment = new RaidenAppointment({
      ...appointmentObj,
      stateUpdate: {
        ...appointmentObj.stateUpdate,
        closing_participant: wrongClosingParticipant,
      },
    });
  ...
  it("throws for wrong closing participant sig", async () => {
    ...
    const appointment = new RaidenAppointment({
      ...
      stateUpdate: {
        ...appointmentObj.stateUpdate,
        closing_signature: incorrectClosingParticipantSig,
      },
    });
  ...
  it("throws for incorrect non_closing participant", async () => {
    const wrongNonClosingParticipant =
      "0xdddEC4D561eE68F37855fa3245Cb878b10Eb1fA0";

    const appointment = new RaidenAppointment({
      ...appointmentObj,
      stateUpdate: {
        ...appointmentObj.stateUpdate,
        non_closing_participant: wrongNonClosingParticipant,
      },
    });
    const inspector = new RaidenInspector(minimumDisputePeriod, provider);
    expect(inspector.checkInspection(appointment)).to.be.rejected;
  ...
  it("throws for wrong non_closing participant sig", async () => {
    ...
    const appointment = new RaidenAppointment({
      ...
      stateUpdate: {
        ...appointmentObj.stateUpdate,
        non_closing_signature: incorrectNonClosingParticipantSig,
      },
    });
    ...
    const appointment = new RaidenAppointment({
      ...
      stateUpdate: {
        ...
        token_network_identifier:
          appointmentObj.stateUpdate.non_closing_participant,
      },
    });
    const inspector = new RaidenInspector(minimumDisputePeriod, provider);
    expect(inspector.checkInspection(appointment)).to.be.rejected;
  ...
  it("throws for wrong code returned", async () => {
    const appointment = new RaidenAppointment({
      ...
      stateUpdate: {
        ...
        token_network_identifier:
          appointmentObj.stateUpdate.closing_participant,
      },
    });
    const inspector = new RaidenInspector(minimumDisputePeriod, provider);
    expect(inspector.checkInspection(appointment)).to.be.rejected;
