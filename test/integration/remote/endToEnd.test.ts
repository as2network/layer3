...
describe("Integration", function () {
  ...
  before(async () => {
    ...
    const config: IArgConfig = {
      ...
      responderKey:
        "0x549a24a594a51f0bea8655a80c01689206a811120e2b28683d6b202f096a2049",
      receiptKey:
        "0x549a24a594a51f0bea8655a80c01689206a811120e2b28683d6b202f096a2049",
    };
    pisa = new PisaContainer(
      dockerClient,
      `pisa-${newId()}`,
      config,
      3000,
  ...
  it("End to end", async () => {
    ...
    const channelContract = await channelContractFactory.deploy(
      [key0.account, key1.account],
      disputePeriod
    );
    // pisa needs some time to initialise -and for some reason the contract needs time to set
    await wait(4000);

    const hashState = ethers.utils.keccak256(
      ethers.utils.toUtf8Bytes("face-off")
    );
    const round = 1;
    ...
    const sig0 = await key0.wallet.signMessage(
      ethers.utils.arrayify(setStateHash)
    );
    const sig1 = await key1.wallet.signMessage(
      ethers.utils.arrayify(setStateHash)
    );
    ...
    const res = await request.post(
      `http://localhost:${pisa.config.hostPort}/appointment`,
      {
        json: appointment,
      }
    );
