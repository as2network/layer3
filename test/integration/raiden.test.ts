...
let alice: ChildProcess,
  bob: ChildProcess,
  pisa: ChildProcess,
  daemon: ChildProcess,
  autominer: ChildProcess;
...
describe("Raiden end-to-end tests for scenario 2 (with Pisa)", function () {
  ...
  const startRaidenNode = async (
    ...
    const logStream = await fse.createWriteStream(
      `${pisaRoot}/logs/${name}.test.log`,
      { flags: "a" }
    );
  ...
  beforeEach(async () => {
    if (!fse.existsSync(`${pisaRoot}/logs`))
      await fse.mkdirp(`${pisaRoot}/logs`);
    if (fse.existsSync(`${demoDir}/build/docker`))
      await fse.remove(`${demoDir}/build/docker`);
    ...
    const parityLogStream = await fse.createWriteStream(
      `${pisaRoot}/logs/parity.test.log`,
      { flags: "a" }
    );
    ...
    const autominerLogStream = await fse.createWriteStream(
      `${pisaRoot}/logs/autominer.test.log`,
      { flags: "a" }
    );
    ...
    const pisaLogStream = await fse.createWriteStream(
      `${pisaRoot}/logs/pisa.test.log`,
      { flags: "a" }
    );
    ...
    const daemonLogStream = await fse.createWriteStream(
      `${pisaRoot}/logs/daemon.test.log`,
      { flags: "a" }
    );
    ...
    console.log(
      ...
      aliceTokenBalance_start,
      bobTokenBalance_start
    );
  });
  ...
  afterEach(async () => {
    console.log("CLEANUP");
    // Cleanup on exit
    const killProcesses = subprocesses.map(
      (child) => new Promise((resolve) => kill(child.pid, "SIGKILL", resolve))
    );
    try {
  ...
  it("completes scenario 2 correctly", async () => {
    ...
    while (true) {
      ...
      try {
        ...
        console.log(
          `Payment succeeded after ${paymentAttempts} attempt${
            paymentAttempts === 1 ? "" : "s"
          }.`
        );
        break;
    ...
    const aliceTokenBalance_end = await tokenContract.balanceOf(
      `0x${aliceAddr}`
    );
    ...
    assert(
      aliceTokenBalance_start.add(paymentAmount).eq(aliceTokenBalance_end),
      "Pisa saved the day."
    );
  });
});
