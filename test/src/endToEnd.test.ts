...
describe("End to end", () => {
  let player0: string,
    ...
    round: number,
    provider: ethers.providers.Web3Provider = new ethers.providers.Web3Provider(
      ganache
    );
  ...
  it("inspect and watch a contract", async () => {
    ...
    const store = new AppointmentStore(
      db,
      new Map([
        [ChannelType.Kitsune, (obj: any) => new KitsuneAppointment(obj)],
      ])
    );
    await store.start();
    ...
    const player0Contract = channelContract.connect(
      provider.getSigner(player0)
    );
