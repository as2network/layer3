...
import {
  ...
  ExponentialGasCurve,
  GasPriceEstimator,
} from "../../../src/responder/gasPriceEstimator";
...
describe("ExponentialCurve", () => {
  ...
  it("getY returns new point", () => {
    ...
    expect(Math.fround(curve.getY(2))).to.equal(
      Math.fround(Math.pow(Math.E, 2))
    );
  });
});
...
describe("ExponentialGasCurve", () => {
  it("constructor does not accept negative gasPrice", () => {
    expect(() => new ExponentialGasCurve(new BigNumber(-1))).to.throw(
      ArgumentError
    );
  });
  ...
  it("max initial gas gives max curve", () => {
    ...
    expect(
      expGasCurve.getGasPrice(ExponentialGasCurve.MAX_BLOCKS + 20).toNumber()
    ).to.equal(Math.round(expCurve.getY(ExponentialGasCurve.MAX_BLOCKS + 20)));
  });
  ...
  it("getGasPrice returns the max gas price for less blocks than max blocks", () => {
    ...
    const expGasCurve = new ExponentialGasCurve(new BigNumber(y2));
    expect(
      expGasCurve.getGasPrice(ExponentialGasCurve.MAX_BLOCKS - 1).toNumber()
    ).to.equal(ExponentialGasCurve.MAX_GAS_PRICE);
  });
...
describe("GasPriceEstimator", () => {
  it("estimate", async () => {
    ...
    when(mockedBlockCache.head).thenReturn({
      hash: "hash1",
      parentHash: "hash2",
      number: 1,
    });
    ...
    const expectedValue = new ExponentialGasCurve(currentGasPrice).getGasPrice(
