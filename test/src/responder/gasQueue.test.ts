...
describe("GasQueue", () => {
  ...
  it("add does append for lowest gas", () => {
    ...
    expect(appendedQueue.queueItems[2].idealGasPrice).to.equal(
      request.idealGasPrice
    );
    expect(appendedQueue.queueItems[2].nonceGasPrice).to.equal(
      request.idealGasPrice
    );
  ...
  it("add does replace for middle gas", () => {
    ...
    expect(appendedQueue.queueItems[1].idealGasPrice).to.equal(
      request.idealGasPrice
    );
    ...
    expect(appendedQueue.queueItems[2].nonce).to.equal(3);
    expect(appendedQueue.queueItems[2].request).to.equal(
      queue.queueItems[1].request
    );
    expect(appendedQueue.queueItems[2].idealGasPrice).to.equal(
      queue.queueItems[1].idealGasPrice
    );
    ...
    expect(appendedQueue.queueItems[3].request).to.equal(
      queue.queueItems[2].request
    );
    expect(appendedQueue.queueItems[3].idealGasPrice).to.equal(
      queue.queueItems[2].idealGasPrice
    );
  ...
  it("consume to remove queue item", () => {
    ...
    expect(consumedQueue.queueItems[0].idealGasPrice).to.equal(
      items[0].request.idealGasPrice
    );
    expect(consumedQueue.queueItems[0].nonceGasPrice.toNumber()).to.equal(115);

    // // unchanged next item
    expect(consumedQueue.queueItems[1].nonce).to.equal(
      queue.queueItems[2].nonce
    );
    expect(consumedQueue.queueItems[1].request).to.equal(
      queue.queueItems[2].request
    );
    expect(consumedQueue.queueItems[1].idealGasPrice).to.equal(
      queue.queueItems[2].idealGasPrice
    );
    expect(consumedQueue.queueItems[1].nonceGasPrice).to.equal(
      queue.queueItems[2].nonceGasPrice
    );

    expect(consumedQueue.queueItems.length).to.equal(
  ...
  it("consume to throw for unknown identifier", () => {
    ...
    expect(() => queue.consume(createIdentifier("data3", "to3"))).to.throw(
      ArgumentError
    );
  });
