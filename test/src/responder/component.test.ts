...
const newIdentifierAndTransaction = (
  blockNumber: number,
  data: string,
  from: string,
  nonce: number
...
describe("ResponderAppointmentReducer", () => {
  ...
  it("getInitialState sets pending tx", () => {
    const reducer = new ResponderAppointmentReducer(
      blockCache,
      txID1.identifier,
      appointmentId1,
      from1
...
const makePendingAppointmentState = (
...
const setupState = (
  states: ResponderAppointmentAnchorState[],
  blockNumber: number
): ResponderAnchorState => {
...
describe("MultiResponderComponent", () => {
  ...
  it("handleChanges reEnqueues all pending items", async () => {
    ...
    const component = new MultiResponderComponent(
      multiResponder,
      blockCache,
