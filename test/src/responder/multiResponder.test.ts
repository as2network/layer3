...
describe("MultiResponder", () => {
  ...
  beforeEach(() => {
    ...
    when(errorGasEstimatorMock.estimate(anything())).thenThrow(
      new Error("Gas test error")
    );
  ...
  it("txMined does replace", async () => {
    ...
    expect(itemAfterMined.request.identifier).to.deep.equal(
      itemAfterReplace.request.identifier
    );
  ...
  it("reEnqueueMissingItems does issue new transactions", async () => {
    ...
    expect(replacedTransactions[0].request.identifier).to.equal(
  ...
  it("reEnqueueMissingItems does replace transactions", async () => {
    ...
    await responder.startResponse(appointmentId2, responseData2);
    const item2 = responder.respondedTransactions.get(appointmentId2)!
      .queueItem;
    ...
    expect(replacedTransactions[0].request.identifier).to.equal(
      item2.request.identifier
    );
    expect(replacedTransactions[0].nonce).to.equal(item.nonce);
    expect(replacedTransactions[1].request.identifier).to.equal(
      item.request.identifier
    );
