...
describe("hasLogMatchingEventFilter", () => {
  ...
  it("returns true if an appropriate log is present", () => {
    expect(hasLogMatchingEventFilter(blockHasLogs, { address, topics })).to.be
      .true;
    expect(
      hasLogMatchingEventFilter(blockHasLogs, {
        address: addressDifferentCase,
        topics,
      }),
  ...
  it("returns false if an appropriate log is not present", () => {
    expect(hasLogMatchingEventFilter(blockDoesNotHaveLogs, { address, topics }))
      .to.be.false;
    expect(
      hasLogMatchingEventFilter(blockHasLogs, {
  ...
  it("throws ArgumentError if no address is provided", () => {
    expect(() => hasLogMatchingEventFilter(blockHasLogs, { topics })).to.throw(
      ArgumentError
    );
  });
  it("throws ArgumentError if no topics member is present in filter", () => {
    expect(() => hasLogMatchingEventFilter(blockHasLogs, { address })).to.throw(
      ArgumentError
    );
  });
});
