...
describe("GarbageCollector", () => {
  ...
  const createMockAppointment = (
    id: string,
    ethersEventFilter: ethers.EventFilter
  ) => {
  ...
  const appointmentInstance1 = createMockAppointment(
    appointmentId1,
    eventFilter
  );
  const appointmentInstance2 = createMockAppointment(
    appointmentId2,
    eventFilter
  );
  const errorSubscriberAppointment = createMockAppointment(
    errorAppointmentID,
    eventFilter
  );
  const errorStoreRemoveByIdAppointment = createMockAppointment(
    errorRemoveByIdId,
    eventFilter
  );
  ...
  when(mockedStore.removeById(errorStoreRemoveByIdAppointment.id)).thenReject(
    new Error("Remove failed.")
  );
  when(
    mockedStore.getExpiredSince(appointment1Expired - confirmationCount)
  ).thenCall(() => [appointmentInstance1].values());

  when(
    mockedStore.getExpiredSince(appointment2Expired - confirmationCount)
  ).thenCall(() => [appointmentInstance2].values());
  when(
    mockedStore.getExpiredSince(bothAppointmentsExpired - confirmationCount)
  ).thenCall(() => [appointmentInstance1, appointmentInstance2].values());
  when(
    mockedStore.getExpiredSince(nothingExpired - confirmationCount)
  ).thenCall(() => [].values());
  ...
  when(
    mockedStore.getExpiredSince(errorStoreExpired - confirmationCount)
  ).thenThrow(new Error("Exceptional expired error."));
  when(
    mockedStore.getExpiredSince(
      errorSubscriberAppointmentExpired - confirmationCount
    )
  ).thenCall(() => [errorSubscriberAppointment].values());
  when(
    mockedStore.getExpiredSince(errorRemoveByIdExpiredBlock - confirmationCount)
  ).thenCall(() => [errorStoreRemoveByIdAppointment].values());
