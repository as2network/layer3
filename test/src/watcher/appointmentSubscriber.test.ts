...
import {
  ...
  IAppointmentListener,
} from "../../../src/watcher/appointmentSubscriber";
...
describe("AppointmentSubscriber", () => {
  ...
  const testListener = () => {
    throw new Error(
      "Should not be fired during subscription / non subscription."
    );
  };

  let provider: ethers.providers.Web3Provider,
    subscriber: AppointmentSubscriber;
  ...
  it("subscribeOnce correctly subcribes 1 appointment", () => {
    ...
    assert.strictEqual(
      (provider.listeners(eventFilter1)[0] as IAppointmentListener)
        .appointmentId,
      appointmentId1
    );
  });
  ...
  it("subscribeOnce correctly subcribes 2 different appointments", () => {
    ...
    assert.strictEqual(
      (provider.listeners(eventFilter2)[0] as IAppointmentListener)
        .appointmentId,
      appointmentId2
    );
  });
