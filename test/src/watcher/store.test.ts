...
describe("Store", () => {
  ...
  beforeEach(async () => {
    ...
    const map = new Map([
      [
        "test" as ChannelType,
        (obj: any) =>
          getAppointment(obj.id, obj.stateLocator, obj.endBlock, obj.nonce)
            .object,
      ],
    ]);
  ...
  it("addOrUpdate does add appointment", async () => {
    ...
    const storedAppointments = [
      ...store.getExpiredSince(appointment1.object.endBlock + 1),
    ];
  ...
  it("addOrUpdate does add multiple appointments", async () => {
    ...
    expect(storedAppointments).to.deep.equal([
      appointment1.object,
      appointment2.object,
    ]);

    const dbAppointment1 = await db.get(appointment1.object.id);
    expect(dbAppointment1).to.deep.equal(
      appointment1.object.getDBRepresentation()
    );
    const dbAppointment2 = await db.get(appointment2.object.id);
    expect(dbAppointment2).to.deep.equal(
      appointment2.object.getDBRepresentation()
    );
  });
  ...
  it("addOrUpdate does update older appointment", async () => {
    ...
    const storedAppointments = [
      ...store.getExpiredSince(appointment2.object.endBlock + 1),
    ];
  ...
  it("removeById does remove appointment", async () => {
    ...
    expect([
      ...(await store.getExpiredSince(appointment1.object.endBlock + 1)),
    ]).to.deep.equal([]);
  ...
  it("removeById does not remove non-existant appointment", async () => {
    ...
    expect([
      ...(await store.getExpiredSince(appointment1.object.endBlock + 1)),
    ]).to.deep.equal([appointment1.object]);
    const dbAppointment1 = await db.get(appointment1.object.id);
    expect(dbAppointment1).to.deep.equal(
      appointment1.object.getDBRepresentation()
    );
  ...
  it("removeById does not remove other appointments", async () => {
    ...
    expect([
      ...(await store.getExpiredSince(appointment1.object.endBlock + 1)),
    ]).to.deep.equal([appointment2.object]);
  });
  ...
  it("expiredSince fetches items with end block less than supplied", async () => {
    ...
    expect([...(await store.getExpiredSince(5))]).to.deep.equal([
      appointment1.object,
    ]);
  });

  const expectAppointmentEquals = (
    actual: IEthereumAppointment,
    expected: IEthereumAppointment
  ) => {
  ...
  it("startup does load all appointments", async () => {
    ...
    await testDB.put(
      appointment1.object.id,
      appointment1.object.getDBRepresentation()
    );
    await testDB.put(
      appointment2.object.id,
      appointment2.object.getDBRepresentation()
    );
    await testDB.put(
      appointment3.object.id,
      appointment3.object.getDBRepresentation()
    );
    ...
    map.set(
      "test",
      (obj: any) =>
        getAppointment(obj.id, obj.stateLocator, obj.endBlock, obj.nonce).object
    );
    const testStore = new AppointmentStore(testDB, map);
    await testStore.start();

    let expired = [
      ...(await testStore.getExpiredSince(appointment3.object.endBlock + 1)),
    ];
  ...
  it("getAll returns all appointments", async () => {
    const appointment1 = getAppointment("id1", "stateLocator1", 1, 1);
    const appointment2 = getAppointment(
      "id2",
      "stateLocator2",
      500000000000,
      1
    ...
    const appointment3 = getAppointment("id3", "stateLocator3", 10, 1);
    const appointment3A = getAppointment(
      "id4",
      "stateLocator3",
      1000000000000000,
      2
