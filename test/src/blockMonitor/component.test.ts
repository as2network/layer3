...
import {
  ...
  StateReducer,
  BlockNumberReducer,
} from "../../../src/blockMonitor/component";
...
class TestAnchorStateReducer
  implements StateReducer<TestAnchorState, IBlockStub> {
  constructor(private readonly startValue: number) {}
  getInitialState = (block: IBlockStub) => ({
    someNumber: this.startValue + block.number,
  });
...
describe("MappedStateReducer", () => {
  ...
  it("reduce computes reduces state", () => {
    ...
    expect(reducedState.someNumber).to.equal(
      10 + blocks[1].number + blocks[2].number
    );
  });
  ...
  it("reduce calls getInitialState if a new object id is added to the collection", () => {
    ...
    const items = new Map<string, TestAnchorState>();
    items.set(objects[0].id, {
      someNumber: objects[0].value + blocks[0].number,
    });
    items.set(objects[1].id, {
      someNumber: objects[1].value + blocks[0].number,
    });
    const initialState = { items };
    ...
    const expectedMap = new Map<string, TestAnchorState>();
    expectedMap.set(objects[0].id, {
      someNumber: objects[0].value + blocks[0].number + blocks[1].number,
    });
    expectedMap.set(objects[1].id, {
      someNumber: objects[1].value + blocks[0].number + blocks[1].number,
    });
    expectedMap.set(objects[2].id, {
      someNumber: objects[2].value + blocks[1].number,
    }); // no block[0]!
