...
describe("BlockProcessor", () => {
  ...
  function emitBlockHash(hash: string) {
    ...
    while (curBlockHash in blocksByHash) {
      ...
      when(mockProvider.getBlock(curBlock.number)).thenResolve(
        curBlock as ethers.providers.Block
      );
  ...
  beforeEach(async () => {
    ...
    for (const [hash, blockStub] of Object.entries(blocksByHash)) {
      when(mockProvider.getBlock(hash)).thenResolve(
        blockStub as ethers.providers.Block
      );
    }
    ...
    when(
      mockProvider.on(anything(), anything())
    ).thenCall((arg0: any, arg1: any) => eventEmitter.on(arg0, arg1));
    when(
      mockProvider.once(anything(), anything())
    ).thenCall((arg0: any, arg1: any) => eventEmitter.once(arg0, arg1));
    when(
      mockProvider.removeListener(anything(), anything())
    ).thenCall((arg0: any, arg1: any) =>
  ...
  it("adds to the blockCache all ancestors until a known block", async () => {
    ...
    for (let i = 1; i <= 5; i++) {
      subscribers.push(
        createNewBlockSubscriber(blockProcessor, blockCache, `a${i}`)
      );
    }
  ...
  it("adds both chain until the common ancestor if there is a fork", async () => {
    ...
    for (let i = 3; i <= 6; i++) {
      subscribersB.push(
        createNewBlockSubscriber(blockProcessor, blockCache, `b${i}`)
      );
    }
  ...
  it("adds both chain until the common ancestor if there is a fork", async () => {
    ...
    for (let i = 1; i <= 10; i++) {
      subscribersC.push(
        createNewBlockSubscriber(blockProcessor, blockCache, `c${i}`)
      );
    }
