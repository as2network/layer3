...
function generateBlocks(
  ...
  for (let height = initialHeight; height < initialHeight + nBlocks; height++) {
    ...
    const block = {
      ...
      parentHash:
        rootParentHash != null && height === initialHeight
          ? rootParentHash
          : `hash-${chain}-${height - 1}`,
...
describe("BlockCache", () => {
  ...
  it("maxHeight is equal to the height of the highest added block", () => {
    ...
    for (const block of generateBlocks(
      blocksAdded - 1,
      initialHeight,
      "forkedchain"
    )) {
  ...
  it("ancestry iterates over all the ancestors", () => {
    ...
    generateBlocks(5, 3, "fork", blocks[1].hash).forEach((block) =>
      bc.addBlock(block)
    );
  ...
  it("findAncestor returns the nearest ancestor that satisfies the predicate", () => {
    ...
    const result = bc.findAncestor(
      headBlock.hash,
      (block) => block.hash == blocks[2].hash
    );
  ...
  it("findAncestor returns self if satisfies the predicate", () => {
    ...
    const result = bc.findAncestor(
      headBlock.hash,
      (block) => block.hash == headBlock.hash
    );
    expect(result).to.not.be.null;
  ...
  it("findAncestor returns self if satisfies the predicate", () => {
    ...
    const result = bc.findAncestor(
      headBlock.hash,
      (block) => block.hash == "notExistingHash"
    );
  ...
  it("getOldestAncestorInCache throws ArgumentError for a hash not in cache", () => {
    ...
    expect(() => bc.getOldestAncestorInCache("notExistingHash")).to.throw(
      ArgumentError
    );
  });
...
describe("getConfirmations", () => {
  ...
  it("correctly computes the number of confirmations for a transaction", () => {
    ...
    const headBlock = blocks[blocks.length - 1];
    expect(
      getConfirmations(bc, headBlock.hash, blocks[0].transactionHashes[0])
    ).to.equal(blocks.length);
    expect(
      getConfirmations(bc, headBlock.hash, blocks[1].transactionHashes[0])
    ).to.equal(blocks.length - 1);
  });
  ...
  it("correctly returns 0 confirmations if transaction is not known", () => {
    ...
    const headBlock = blocks[blocks.length - 1];
    expect(getConfirmations(bc, headBlock.hash, "nonExistingTxHash")).to.equal(
      0
    );
  });
  ...
  it("throws ArgumentError if no block with the given hash is in the BlockCache", () => {
    ...
    expect(() =>
      getConfirmations(
        bc,
        "nonExistingBlockHash",
        headBlock.transactionHashes[0]
      )
