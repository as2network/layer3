...
let setup = async (creationCount: number) => {
  ...
  for (let contractIndex = 0; contractIndex < creationCount; contractIndex++) {
    let contract = await channelContractFactory.deploy(
      [account0, account1],
      disputePeriod
    );
...
let execute = async (timeToWait: number) => {
  ...
  for (let index = 0; index < channelContracts.length; index++) {
    ...
    channelContract.on(setStateEvent, () => {
      ...
      console.log(
        `Set state acknowleged for contract ${channelContract.address}.`
      );
    ...
    console.log(
      `Appointment request made for contract: ${channelContract.address}.`
    );
  }
  ...
  await waitForPredicate(
    eventReceived,
    (s) => s.length == channelContracts.length && s.reduce((a, b) => a && b),
    400
  );
...
if (process.argv.length !== 4) {
  console.error(
    "Incorrect arguments supplied. Supply number of requests to make and the time to wait (ms)."
  );
  process.exit(1);
}
...
flow(requestCount, waitTimeMs).then(
  () => {
